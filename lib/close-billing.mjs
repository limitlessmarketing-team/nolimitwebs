...
export function createBillingService(env, dependencies = {}) {
  ...
  async function publish(p, message) {
    ...
    const values = activityFields(proposalFields, {
      ...
      deposit: p.kind === HOSTING_FLOW && p.setupAccepted ? p.id : p.depositId || null, final: p.finalId || null, subscription: p.subscriptionId || null,
    });
  ...
  async function createProposal(activity) {
    ...
    const build = dollarsToCents(field(activity, proposalFields.build), { whole: true });
    const hosting = dollarsToCents(field(activity, proposalFields.hosting));
    requireMatch(build > 0 || hosting > 0, 'Enter a positive monthly hosting price for a hosting-only proposal.');
    const choice = config.proposalFields.paymentPlan ? field(activity, config.proposalFields.paymentPlan) : null;
    ...
    await store.create({ id: activity.id, leadId: activity.lead_id, title, build, hosting, paymentPlan, stage: 'creating' });
    await store.withLock(activity.id, async (p, tx) => {
      requireMatch(p.leadId === activity.lead_id && p.title === title && p.build === build && p.hosting === hosting && (p.paymentPlan || 'deposit_50') === paymentPlan,
      ...
      if (p.build === 0) return hostingOnly.create(p, tx);
      ...
      const link = await tx.post('payment-link', 'payment_links', {
        ...
        'invoice_creation[enabled]': 'true', 'invoice_creation[invoice_data][description]': full ? 'Website build — full payment' : 'Website build — 50% deposit',
  ...
  async function recordDeposit(sessionId) {
    ...
    await store.withLock(projectId, async (p, tx) => {
      ...
      requireMatch((session.metadata.payment_plan || 'deposit_50') === (p.paymentPlan || 'deposit_50') &&
        session.metadata.build_total_cents === String(p.build) && session.metadata.hosting_monthly_cents === String(p.hosting), 'Payment does not match the agreed plan and prices.');
  ...
  async function validateLaunch(p) {
    ...
    requireMatch(invoice.status === 'paid' && idOf(invoice.customer) === p.customerId && invoice.currency === 'usd' &&
      invoice.amount_paid === upfrontAmount(p) && invoice.subtotal_excluding_tax === upfrontAmount(p) &&
    ...
    requireMatch(intent.status === 'succeeded' && idOf(intent.customer) === p.customerId && intent.amount_received === upfrontAmount(p) &&
  ...
  async function launch(activity) {
    ...
    await store.withLock(projectId, async (p, tx) => {
      ...
      await refresh(p, tx);
      const result = `Launch recorded. ${p.paymentPlan === FULL_PLAN ? 'Website already paid in full; no build charge at launch.' : `Final invoice: ${p.finalId}.`} ${p.subscriptionId ? `Hosting subscription: ${p.subscriptionId}.` : 'No recurring hosting.'} See the proposal activity for current payment status.`;
      if (field(activity, launchFields.result) !== result) await close(activityPath(activity.id), activityFields(launchFields, { result }));
    });
  }
  ...
  const checkActivity = async p => {
    ...
    requireMatch(a.status === 'published' && a.lead_id === p.leadId && a.organization_id === config.organizationId &&
      ...
      dollarsToCents(field(a, proposalFields.build), { whole: true }) === p.build &&
      dollarsToCents(field(a, proposalFields.hosting)) === p.hosting, 'Proposal changed or was removed; contact our team.');
  };
