import { proposalCancellation, cancellationBlocks, CANCELED_MESSAGE } from './cancel-proposal.mjs';
...
function createSingleBillingService(env, dependencies = {}) {
  ...
  async function createProposal(activity) {
    ...
    await store.withLock(activity.id, async (p, tx) => {
      ...
      if (cancellationBlocks(p)) return refresh(p, tx);
  ...
  async function launch(activity) {
    ...
    await store.withLock(projectId, async (p, tx) => {
      requireMatch(!cancellationBlocks(p), 'Proposal canceled or cancellation under review. Review Stripe before launching.');
  ...
  async function refresh(p, tx) {
    ...
    if (p.build === 0 && p.domainAmount) message = p.depositId ? (p.stage === 'launched' ? 'Website launched. Domain registration paid.' : 'Domain registration paid — ready for website launch.') : 'Proposal ready — awaiting domain payment.';
    if (p.finalId) {
    ...
    if (p.cancellation) message = p.cancellation.status === 'canceled' ? 'Proposal canceled — old link and open checkouts disabled.' : 'Proposal cancellation pending or needs review — check the cancellation action before sending a replacement.';
  ...
  const checkActivity = async p => {
    requireMatch(!cancellationBlocks(p), CANCELED_MESSAGE);
  ...
  return { onClose, onStripe, publishCancellation: (p,message,tx) => publish(p,message,tx), hostingProposal: hostingOnly.publicProposal, hostingCheckout: hostingOnly.checkout };
}
...
export function createBillingService(env, dependencies = {}) {
  ...
  const cancel = config.cancelType ? proposalCancellation({ config, api, close, store, now: dependencies.now || (() => Date.now()),
    publish: (p,message,tx) => forProject(p).publishCancellation(p,message,tx) }) : null;
  const onClose = async payload => {
    ...
    const activity = await readEventActivity(close, event.object_id);
    if (!activity) return;
    if (cancel && activity.custom_activity_type_id === config.cancelType) return cancel(payload);
