...
function createSingleBillingService(env, dependencies = {}) {
  ...
  async function createProposal(activity) {
    ...
    await store.create({ id: activity.id, leadId: activity.lead_id, title, build, hosting, paymentPlan, proposalType: config.proposalType, stage: 'creating' });
    await store.withLock(activity.id, async (p, tx) => {
      requireMatch((p.proposalType || config.legacyProposalType || config.proposalType) === config.proposalType && p.leadId === activity.lead_id && p.title === title && p.build === build && p.hosting === hosting && (p.paymentPlan || 'deposit_50') === paymentPlan,
