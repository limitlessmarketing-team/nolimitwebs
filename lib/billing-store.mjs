...
export class BillingStore {
  ...
  async findHostingToken(token) {
    const row = await this.db.prepare("SELECT state FROM close_billing_projects WHERE mode = ? AND json_extract(state, '$.linkId') = ?")
      .bind(this.mode, token).first();
    return row ? JSON.parse(row.state) : null;
  }
