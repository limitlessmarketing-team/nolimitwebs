...
export async function readProposal(id, api = stripe, mode = envVar('STRIPE_MODE')) {
  ...
  if (link.metadata?.flow !== FLOW || link.metadata.authorization_version !== TERMS || !expectedMode(link.livemode, mode)) throw new Error('Unavailable proposal');
  ...
  if (lines.has_more || lines.data?.length !== 1 || item?.quantity !== 1 ||
      ...
      !Number.isSafeInteger(deposit) || deposit <= 0 ||
