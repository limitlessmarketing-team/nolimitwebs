...
export function billingConfig(env) {
  ...
  if (config.cancelType) {
    if (!/^actitype_[A-Za-z0-9]+$/.test(config.cancelType) || [config.proposalType, config.launchType, ...(config.paths || []).flatMap(p => [p.proposalType,p.launchType])].includes(config.cancelType)) throw new Error('Invalid cancellation type');
    for (const name of ['proposal','authorization','result']) if (!/^cf_[A-Za-z0-9]+$/.test(config.cancelFields?.[name] || '')) throw new Error('Missing cancellation field');
  }
