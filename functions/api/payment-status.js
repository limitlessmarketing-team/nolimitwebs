...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    return reply({ paid, buildTotal: Number(session.metadata.build_total_cents), domainAmount: Number(session.metadata.domain_amount_cents || 0), domainName: session.metadata.domain_name || '', paymentPlan: session.metadata.payment_plan || 'deposit_50', monthlyHosting: Number(session.metadata.hosting_monthly_cents), amount: paid ? session.amount_total : null, invoiceUrl });
